...
- All QuantLab labs hard-reject women fixtures before fixture-level odds/context/statistics calls.
- Legacy women fixtures already stored in QuantLab are skipped by history backfill and shadow evaluation.
- The global `fixtures?date=...` discovery call remains shared for the whole date shard, so the hard stop eliminates downstream women-specific API spend rather than the unavoidable global shard request.
